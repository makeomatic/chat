...
do
  ...
  # docker build --build-arg VERSION=$tag --build-arg TARGET_DB=${dbtag} --tag ${rmitags} docker/tinode
  echo docker build --build-arg VERSION=$version --build-arg TARGET_DB=${dbtag} --tag ${rmitags} docker/tinode
done